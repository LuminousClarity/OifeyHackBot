...
local pack = util.emoji.get("database/sw/emoji.json")
...
Character.section = almanac.get("database/sw/rev.json")

Character.helper_job_base = true
Character.helper_job_growth = true
Character.helper_job_cap = true

Character.helper_portrait = "database/sw/images"
...
function Character:apply_color(infobox)
    if self.route ~= nil and box_color[self.route] ~= nil then
        ...
        infobox:image("icon", string.format("database/sw/icon_%s.png", self.route) )
    end
end
---------------------------------------------------
-- Job --
...
Job.section = almanac.get("database/sw/job.json")
...
Skill.section = almanac.get("database/sw/skill.json")
...
Item.section = almanac.get("database/sw/item.json")
...
Redirect.section = almanac.get("database/sw/redirect.json")
...
Silentwaves.__index = Silentwaves
setmetatable(Silentwaves, Character)

Silentwaves.section = almanac.get("database/sw/rev.json")

Silentwaves.route = "sw"
