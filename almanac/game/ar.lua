...
Character.section = almanac.get("database/ar/ar.json")

Character.helper_job_base = true
Character.helper_job_growth = true
Character.helper_job_cap = true

Character.helper_portrait = "database/ar/images"
...
function Character:apply_color(infobox)
    if self.route ~= nil and box_color[self.route] ~= nil then
        ...
        infobox:image("icon", string.format("database/ar/icon_%s.png", self.route) )
    end
end
---------------------------------------------------
-- Job --
...
Job.section = almanac.get("database/ar/job.json")
...
Skill.section = almanac.get("database/ar/skill.json")
...
Item.section = almanac.get("database/ar/item.json")
...
Redirect.section = almanac.get("database/ar/redirect.json")
...
-- Awakening Remake
Awakeningremake.__index = Awakeningremake
setmetatable(Awakeningremake, Character)

Awakeningremake.section = almanac.get("database/ar/ar.json")

Awakeningremake.route = "ar"
...
return {
    ...
    Redirect = Redirect,
	Awakeningremake = Awakeningremake
}
