...
Character.helper_portrait = "database/vow/images"
...
function Character:apply_color(infobox)
    if self.route ~= nil and box_color[self.route] ~= nil then
        ...
        infobox:image("icon", string.format("database/vow/icon_%s.png", self.route) )
    end
end
---------------------------------------------------
-- Job --
...
Job.section = almanac.get("database/vow/job.json")
...
Skill.section = almanac.get("database/vow/skill.json")
...
Item.section = almanac.get("database/vow/item.json")
